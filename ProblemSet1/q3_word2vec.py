...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x * np.power(np.sum(np.power(x, 2), axis = 1, keepdims=True),-0.5)
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    gradPred = np.sum(y_hat.reshape(-1,1)*outputVectors, axis = 0) - outputVectors[target]
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    s = sigmoid(-sample.dot(predicted)).reshape(-1,1)
